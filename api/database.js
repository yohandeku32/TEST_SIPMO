...
export default async function handler(req, res) {
  ...
  try {
    ...
    // Tes koneksi dari browser:
    // https://DOMAIN-VERCEL/api/database
    if (req.method === "GET") {
    ...
    const apiSecret = process.env.SIPMODAG_API_SECRET;
    ...
    // Mencegah beberapa statement sekaligus.
    const sqlWithoutFinalSemicolon = sql.replace(/;\s*$/, "");
    ...
    console.error("SIPMODAG DATABASE API ERROR:", error);

    return res.status(500).json({
      success: false,
      message: error?.message || "Terjadi kesalahan pada database API."
    });
  }
}
